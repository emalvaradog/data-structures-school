...
void ordenarAltura(Persona arr[], int tam){
    for(int i = 0; i < tam; i++){
        for(int j = i+1; j < tam; j++){
            if(arr[j].datos.altura < arr[i].datos.altura){
                Persona cont = arr[i];
                arr[i] = arr[j];
                arr[j] = cont;
            }
...
void ordenarTelefono(Persona arr[], int tam){
    ...
    while ( !ordenado ){
        ordenado=1;
        for(i = 0; i<tam-1; i++){
            if(arr[i].datos.telefono > arr[i+1].datos.telefono)
            {
                Persona aux = arr[i];
                arr[i] = arr[i+1];
                arr[i+1] = aux;
...
void ordenarPais(Persona arr[], int tam){
    ...
    for (int i = 0; i < tam; i++) {
       pivote = arr[i];
        j = i - 1;
        while (j >= 0 && arr[j].datos.pais[0] > pivote.datos.pais[0]) {
          arr[j + 1] = arr[j];
          j--;
        }
        arr[j + 1] = pivote;
...
void principal(){
    int opc = 0;
    while(opc!=6){
        puts("1. Imprimir arreglo");
        puts("2. Ordenar por altura");
        puts("3. Ordenar por nombre");
        puts("4. Ordenar por telefono");
        puts("5. Ordenar por pais");
        puts("6. Salir");
        scanf("%d", &opc);
        switch(opc){
            case 1: 
                imprimirArreglo(personas, tamano);
                break;
            case 2:
                ordenarAltura(personas, tamano);
                break;
            case 3:
                ordenarNombre(personas, tamano);
                break;
            case 4:
                ordenarTelefono(personas, tamano);
                break;
            case 5:
                ordenarPais(personas, tamano);
                break;
            default:
                puts("Bye");
        };
